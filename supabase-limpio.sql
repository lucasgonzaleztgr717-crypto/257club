...
-- Clients
CREATE POLICY "Clients can read own data" ON clients FOR SELECT USING (auth.uid()::text = user_id::text);
CREATE POLICY "Service roles can read all clients" ON clients FOR SELECT USING (EXISTS (SELECT 1 FROM users WHERE users.id = auth.uid() AND users.role IN ('COACH', 'NUTRITIONIST', 'MINDSET_COACH', 'ADMIN')));
CREATE POLICY "Clients can update own data" ON clients FOR UPDATE USING (auth.uid()::text = user_id::text);

-- Coaches
CREATE POLICY "Anyone can read coaches" ON coaches FOR SELECT USING (true);
CREATE POLICY "Coaches can update own data" ON coaches FOR UPDATE USING (auth.uid()::text = user_id::text);
CREATE POLICY "Service roles can insert coaches" ON coaches FOR INSERT WITH CHECK (EXISTS (SELECT 1 FROM users WHERE users.id = auth.uid() AND users.role IN ('COACH', 'ADMIN')));

-- Nutritionists
CREATE POLICY "Anyone can read public nutritionist info" ON nutritionists FOR SELECT USING (true);

-- Mindset coaches
CREATE POLICY "Anyone can read mindset coaches" ON mindset_coaches FOR SELECT USING (true);

-- Workout plans
CREATE POLICY "Clients can read own workout plans" ON workout_plans FOR SELECT USING (auth.uid()::text IN (SELECT user_id::text FROM clients WHERE id = client_id));
CREATE POLICY "Service roles can read all workout plans" ON workout_plans FOR SELECT USING (EXISTS (SELECT 1 FROM users WHERE users.id = auth.uid() AND users.role IN ('COACH', 'ADMIN')));

-- Exercises
CREATE POLICY "Anyone can read exercises" ON exercises FOR SELECT USING (true);
CREATE POLICY "Service roles can manage exercises" ON exercises FOR ALL USING (EXISTS (SELECT 1 FROM users WHERE users.id = auth.uid() AND users.role IN ('COACH', 'ADMIN')));

-- Workout sessions
CREATE POLICY "Users can read own workout sessions" ON workout_sessions FOR SELECT USING (auth.uid()::text = user_id::text);
CREATE POLICY "Users can create own workout sessions" ON workout_sessions FOR INSERT WITH CHECK (auth.uid()::text = user_id::text);
CREATE POLICY "Users can update own workout sessions" ON workout_sessions FOR UPDATE USING (auth.uid()::text = user_id::text);

-- Nutrition plans
CREATE POLICY "Clients can read own nutrition plans" ON nutrition_plans FOR SELECT USING (auth.uid()::text IN (SELECT user_id::text FROM clients WHERE id = client_id));
CREATE POLICY "Nutritionists can read assigned plans" ON nutrition_plans FOR SELECT USING (auth.uid()::text = nutritionist_id::text);

-- Meals
CREATE POLICY "Clients can read own meals" ON meals FOR SELECT USING (auth.uid()::text IN (SELECT user_id::text FROM clients WHERE id IN (SELECT client_id FROM nutrition_plans WHERE id = plan_id)));

-- Measurements
CREATE POLICY "Clients can read own measurements" ON measurements FOR SELECT USING (auth.uid()::text IN (SELECT user_id::text FROM clients WHERE id = client_id));
CREATE POLICY "Clients can create own measurements" ON measurements FOR INSERT WITH CHECK (auth.uid()::text IN (SELECT user_id::text FROM clients WHERE id = client_id));

-- Progress photos
CREATE POLICY "Clients can read own progress photos" ON progress_photos FOR SELECT USING (auth.uid()::text IN (SELECT user_id::text FROM clients WHERE id = client_id));
CREATE POLICY "Clients can create own progress photos" ON progress_photos FOR INSERT WITH CHECK (auth.uid()::text IN (SELECT user_id::text FROM clients WHERE id = client_id));

-- Mindset exercises
CREATE POLICY "Clients can read own mindset exercises" ON mindset_exercises FOR SELECT USING (auth.uid()::text IN (SELECT user_id::text FROM clients WHERE id = client_id));
CREATE POLICY "Clients can create own mindset exercises" ON mindset_exercises FOR INSERT WITH CHECK (auth.uid()::text IN (SELECT user_id::text FROM clients WHERE id = client_id));

-- Appointments
CREATE POLICY "Clients can read own appointments" ON appointments FOR SELECT USING (auth.uid()::text = user_id::text OR (auth.uid()::text IN (SELECT user_id::text FROM clients WHERE id = client_id)));
CREATE POLICY "Service roles can read all appointments" ON appointments FOR SELECT USING (EXISTS (SELECT 1 FROM users WHERE users.id = auth.uid() AND users.role IN ('COACH', 'NUTRITIONIST', 'MINDSET_COACH', 'ADMIN')));
CREATE POLICY "Service roles can create appointments" ON appointments FOR INSERT WITH CHECK (EXISTS (SELECT 1 FROM users WHERE users.id = auth.uid() AND users.role IN ('COACH', 'NUTRITIONIST', 'MINDSET_COACH', 'ADMIN')));

-- Community posts
CREATE POLICY "Anyone can read posts" ON community_posts FOR SELECT USING (true);
CREATE POLICY "Users can create posts" ON community_posts FOR INSERT WITH CHECK (auth.uid()::text = user_id::text);
CREATE POLICY "Users can update own posts" ON community_posts FOR UPDATE USING (auth.uid()::text = user_id::text);
CREATE POLICY "Users can delete own posts" ON community_posts FOR DELETE USING (auth.uid()::text = user_id::text);

-- Chat messages
CREATE POLICY "Anyone can read chat messages" ON chat_messages FOR SELECT USING (true);
CREATE POLICY "Users can create chat messages" ON chat_messages FOR INSERT WITH CHECK (auth.uid()::text = user_id::text);

-- AI conversations
CREATE POLICY "Users can read own AI conversations" ON ai_conversations FOR SELECT USING (auth.uid()::text = user_id::text);
CREATE POLICY "Users can create own AI conversations" ON ai_conversations FOR INSERT WITH CHECK (auth.uid()::text = user_id::text);
CREATE POLICY "Users can update own AI conversations" ON ai_conversations FOR UPDATE USING (auth.uid()::text = user_id::text);

-- Subscriptions
CREATE POLICY "Clients can read own subscriptions" ON subscriptions FOR SELECT USING (auth.uid()::text IN (SELECT user_id::text FROM clients WHERE id = client_id));
CREATE POLICY "Admin can read all subscriptions" ON subscriptions FOR SELECT USING (EXISTS (SELECT 1 FROM users WHERE users.id = auth.uid() AND users.role = 'ADMIN'));
...
CREATE TRIGGER update_mindset_coaches_updated_at BEFORE UPDATE ON mindset_coaches FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE TRIGGER update_mindset_exercises_updated_at BEFORE UPDATE ON mindset_exercises FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
